...
namespace L58.EPAVR
{
    ...
    public abstract class Distribution<T, T_ITEM> 
        where T_ITEM : DistributionItem<T>, new()
    {
        [SerializeField]
        protected List<T_ITEM> items;
        public List<T_ITEM> Items { get { return items; } }

        int nbItems = 0;
        float combinedWeight;
        /*
        public Distribution(Distribution<T, T_ITEM> other) 
        {
            // Create new list
            items = new List<T_ITEM>();
            // Loop through each item in the other distribution and add it
            foreach(DistributionItem<T> item in other.Items)
            {
        ...
        */
        ...
        public void OnItemsChange(bool addedItem = false)
        {
            // On Add Component
            if (items == null)
                return;
