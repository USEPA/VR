...
public class SampleableMesh : MonoBehaviour
{
    ...
    public Vector3[] Vertices { get => mesh.vertices; }
	#endregion

    private void Awake()
    {
		if (!m_isInitialized) CacheReferences();
    }
    ...
    public Vector3 GetNearestPoint(Vector3 position)
    {
		...
		Vector3 objPosition = transform.InverseTransformPoint(position);
		// Build the mesh tree
		VertTriList vt = new VertTriList(mesh);
		KDTree kd = KDTree.MakeFromPoints(Vertices);

		Vector3 meshPt = NearestPointOnMesh(objPosition, Vertices, kd, mesh.triangles, vt);
		return transform.TransformPoint(meshPt);
	}
